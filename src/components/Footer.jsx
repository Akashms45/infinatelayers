  ...
  return (
    <footer id="footer" className="bg-[#151515] text-[#AAAAAA] relative pt-12 pb-6 mt-12">
      ...
      <div className="container-logit relative z-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-x-8 gap-y-12 mb-10 pt-4">

          {/* Column 1: Logo and Address */}
          <div className="col-span-2 md:col-span-2 pr-8 flex flex-col items-start">
            ...
            <p className="text-[14px] leading-relaxed text-[#8C8C9A] max-w-[280px]">
              Infinite Layers<br />
              10th cross ,SS layout A block <br />
          ...
          <div className="col-span-1">
            ...
            <ul className="flex flex-col gap-3 list-none p-0 m-0">
              {["All", "Keychains", "Monuments", "Miniatures", "Statues", "HueForge"].map((filter) => (
                <li key={filter}>
                  <a
                    href={filter === "All" ? "/all-products" : `/all-products?category=${filter}`}
                    onClick={(e) => handleNavClick(e, filter === "All" ? "/all-products" : `/all-products?category=${filter}`)}
                    className="hover:text-[#F5820A] text-[13.5px] transition-colors font-medium text-[#8C8C9A]"
                  >
                    {filter}
                  </a>
        ...
        <div className="border-t border-[#2A2A2A] pt-6 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex gap-4">
            {/* Instagram */}
            <a
              href="https://www.instagram.com/infinatelayers?igsh=dXVmaGYxcGVmMnhj"
              target="_blank"
              rel="noopener noreferrer"
              className="w-10 h-10 rounded-full border border-[#333333] hover:bg-[#F5820A] hover:border-[#F5820A] hover:text-white flex items-center justify-center text-[16px] text-[#8C8C9A] transition-all"
